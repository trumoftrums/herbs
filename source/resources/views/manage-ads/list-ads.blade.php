...
@section('page-title', "QUẢN LÝ ADS")
...
<div class="row tab-search">
    <div class="col-md-2">
        <a href="{{ route('adsadmin.create') }}" class="btn btn-success" id="add-user">
            ...
            @if (count($listNews))
                ...
                @foreach ($listNews as $bd)
                    <tr>
                        <td>{{ $i++ }}</td>
                        <td>{{ $bd->name }}</td>
                        <td><img style="height: 60px" src="{{ url($bd->img)}}"/></td>
                        <td><a href="{{$bd->url}}" target="_blank" >{{$bd->url}}</a> </td>
                        ...
                        <td class="text-center">
                            <a href="{{ route('adsadmin.edit', $bd->id) }}" class="btn btn-primary btn-circle edit" title="Edit"
                                    data-toggle="tooltip" data-placement="top">
                                <i class="glyphicon glyphicon-edit"></i>
                            </a>
                            <a href="{{ route('adsadmin.delete', $bd->id) }}" class="btn btn-danger btn-circle" title="Delete"
                                    data-toggle="tooltip"
                                    data-placement="top"
                                    data-method="DELETE"
