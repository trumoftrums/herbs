...
class AuthController extends Controller
{
    ...
    public function postLogin(LoginRequest $request)
    {
        ...
        if (! Auth::validate($credentials)) {
            ...
            return redirect()->to($this->loginURL . $to)
    ...
    {
        return session('auth.2fa.id') ? view('auth.token') : redirect($this->loginURL);
    }
    ...
    public function postToken(Request $request)
    {
        ...
        if (! Authy::tokenIsValid($user, $request->token)) {
            return redirect()->to($this->loginURL)->withErrors(trans('app.2fa_token_invalid'));
        }
    ...
    public function postRegister(RegisterRequest $request, UserMailer $mailer, RoleRepository $roles)
    {
        ...
        return redirect($this->loginURL)->with('success', $message);
    }
